...
class ExecutionEnv(BaseLOBEnv):
    ...
    def step_env(
        ...
        order_ids = jnp.ones((self.n_actions,), jnp.int32) * (
                    self.trader_unique_id + state.customIDcounter) + jnp.arange(0, self.n_actions)
        ...
        state = EnvState(ordersides[0], ordersides[1], trades[0], state.init_time, time,
                         state.customIDcounter + self.n_actions)
