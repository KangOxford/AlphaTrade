...
# Load data
data_path = 'gymnax_exchange/jaxen/Testing/full_logging_tests/data/exec'
output_dir='gymnax_exchange/jaxen/Testing/full_logging_tests/plotting/exec'
...
def plot_env_midprice_stats(env_data, output_dir):
    ...
    ax.plot(env_data['step'], env_data['mid_price'], label='Mid Price')
    ax.set_xlabel('Step')
    ax.set_ylabel('Price')
    ax.set_title('Midprice over Steps')
    ax.legend()
    ax.grid(True)
    plt.tight_layout()
...
plot_buy_sell_task_size(env_data, trades_df, messages_df, output_dir, trader_id)
