...
def batchify(x: list, num_agents):
    x = jnp.stack([x[a] for a in range(num_agents)], axis=0)
    return x.reshape((num_agents, -1))
...
def make_train(config):
    ...
    def train(rng):
        ...
        runner_state = (
            ...
            env_state,
            obsv,
            jnp.zeros((config["NUM_ACTORS"]), dtype=bool), # last_done
