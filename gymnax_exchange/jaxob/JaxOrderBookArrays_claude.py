...
@jax.jit
def add_order_array(orderside: chex.Array, msg_array: chex.Array) -> chex.Array:
    """Optimized version that takes array instead of dict.
    Adds an order to a given side of the orderbook. 
    Will not add negative quantities to the book. 
        Parameters:
                orderside (Array): Array representing bid or ask side
                msg_array (Array): Message array with data of order to add
                    [type, side, quantity, price, orderid, traderid, time, time_ns]
    ...
    orderside=orderside.at[emptyidx,:]\
                        .set(jnp.array([
                            msg_array[3],  # price
                            jnp.maximum(0,msg_array[2]),  # quantity
                            msg_array[4],  # orderid
                            msg_array[5],  # traderid
                            msg_array[6],  # time
                            msg_array[7]]))\
                        .astype(jnp.int32)
    return _removeZeroNegQuant(orderside)
...
@partial(jax.jit, static_argnums=(0,))
def cancel_order_array(cfg:JAXLOB_Configuration,key:chex.PRNGKey,orderside, msg_array):
    """Optimized array version. Removes quantity of an order from a given side of the orderbook.
    If the resulting order has a remaining quantity of 0 or less it is
    removed entirely. 
    Identifies orders to cancel based on a matching order ID. If there
    is no matching ID, an initial order may also be cancelled provided 
    the price matches. 
        ...
        Parameters:
                orderside (Array): Array representing bid or ask side
                msg_array (Array): Message array identifying order to cancel

        Returns:
                orderside (Array): Orderbook side with cancelled order
    """
    oid_match = (orderside[:, 2] == msg_array[4])  # orderid at index 4
    idx = jnp.where(oid_match, size=1, fill_value=-1)[0][0]
    idx = jax.lax.cond(idx == -1,
                        partial(get_init_id_match_array,cfg,key),
                        lambda a, b: idx,
                        orderside,
                        msg_array,)
    orderside = orderside.at[idx, 1].set(orderside[idx, 1] - msg_array[2])  # quantity at index 2
    return _removeZeroNegQuant(orderside)
...
@partial(jax.jit, static_argnums=(0,))
def get_init_id_match_array(cfg:JAXLOB_Configuration,key:chex.PRNGKey,orderside, msg_array):
    """Array version: Function to check match of an initial message. Used only 
    if the order ID of the message does not match with an 
    existing order. 
    """
    init_id_match = ((orderside[:, 0] == msg_array[3])  # price at index 3
                        & (orderside[:, 2] <= cfg.init_id)
                        & (orderside[:,1]>=msg_array[2]))  # quantity at index 2
    idx = jnp.where(init_id_match, size=1, fill_value=-1)[0][0]
    if cfg.cancel_mode==2 or cfg.cancel_mode==3:
        idx = jax.lax.cond((idx == -1 ),
                        partial(get_random_id_match_array,cfg,key),
                        lambda a, b: idx,
                        orderside,
                        msg_array,)
    else: 
...
@partial(jax.jit, static_argnums=(0,))
def get_random_id_match_array(cfg:JAXLOB_Configuration,key:chex.PRNGKey,orderside, msg_array):
    price_match=((orderside[:, 0] == msg_array[3])  # price at index 3
                    & (orderside[:,1]>=msg_array[2]))  # quantity at index 2
    order_ids=jnp.where(price_match,orderside[:,2],jnp.zeros_like(orderside[:,2]))
    key,_=jax.random.split(key, num=2)
    chosen_id=jax.random.choice(key, order_ids,p=jnp.abs(jnp.sign(order_ids)))
    ...
    if cfg.cancel_mode==3:
        idx = jax.lax.cond((idx == -1 ),
                        partial(get_random_large_id_match_array,cfg,key),
                        lambda a, b: idx,
                        orderside,
                        msg_array,)
    return idx
...
@partial(jax.jit, static_argnums=(0,))
def get_random_large_id_match_array(cfg:JAXLOB_Configuration,key:chex.PRNGKey,orderside, msg_array):
    #This mode will randomly select an order ID that can fill all of the order
    price_match=(orderside[:,1]>=msg_array[2])  # quantity at index 2
    order_ids=jnp.where(price_match,orderside[:,2],jnp.zeros_like(orderside[:,2]))
    key,_=jax.random.split(key, num=2)
    
    # Fix type issue by ensuring order_ids is properly shaped and normalized
    order_ids = jnp.asarray(order_ids)
    probabilities = jnp.abs(jnp.sign(order_ids))
    probabilities = probabilities / (jnp.sum(probabilities) + 1e-8)  # Normalize with epsilon
    
    chosen_id=jax.random.choice(key, order_ids, p=probabilities)
    idx=jnp.where(orderside[:,2]==chosen_id,size=1,fill_value=-1)[0][0]
...
# Array-based version of _match_against_ask_orders
@partial(jax.jit,static_argnums=0)
def _match_against_ask_orders_array(cfg: JAXLOB_Configuration,orderside,qtm,price,trade,agrOID,time,time_ns,agrTID,side):
    """Array-based wrapper for the while loop that gets the top ask order, and
    matches the incoming order against it whilst the 
...
# Array-based version of _match_against_bid_orders
@partial(jax.jit,static_argnums=0)
def _match_against_bid_orders_array(cfg:JAXLOB_Configuration,orderside,qtm,price,trade,agrOID,time,time_ns,agrTID,side):
    """Array-based wrapper for the while loop that gets the top bid order, and
    matches the incoming order against it whilst the 
    _check_before_matching_bid function remains true.
    Returns the new set of bid orders after matching, and the remaining
    quantity to match
...
def doNothing_array(msg_array,askside,bidside,trades):
    """Array-based dummy function for conditional statements to do nothing
    in certain cases. 

        Parameters:
                msg_array (Array): Incoming message array to process.
                askside (Array): All ask orders in book
                bidside (Array): All bid orders in book
                trades (Array): Running count of all occured trades
...
# Array-based cancel functions
@partial(jax.jit,static_argnums=0)
def bid_cancel_array(cfg:JAXLOB_Configuration,key,msg_array,askside,bidside,trades):
    """Array-based function for processing a cancel order on the bid side."""
    return askside,cancel_order_array(cfg,key,bidside,msg_array),trades

@partial(jax.jit,static_argnums=0)
def ask_cancel_array(cfg:JAXLOB_Configuration,key:chex.PRNGKey,msg_array,askside,bidside,trades):
    """Array-based function for processing a cancel order on the ask side."""
    return cancel_order_array(cfg,key,askside,msg_array),bidside,trades

# Placeholder for array-based match function (if needed)
def match_top_order_if_pricematch_array(cfg:JAXLOB_Configuration,msg_array,askside,bidside,trades):
    """Array-based placeholder for match top order function."""
    return askside,bidside,trades
@partial(jax.jit,static_argnums=0)
...
# Array-based version of bid_lim
@partial(jax.jit,static_argnums=0)
def bid_lim_array(cfg:JAXLOB_Configuration,msg_array,askside,bidside,trades):
    """Array-based function for processing a limit order to bid."""
    matchtuple=_match_against_ask_orders_array(cfg,
                                               askside,
                                               msg_array[2],  # quantity
                                               msg_array[3],  # price
                                               trades,
                                               msg_array[4],  # orderid
                                               msg_array[6],  # time
    ...
    msg_array = msg_array.at[2].set(matchtuple[1])
    bids=add_order_array(bidside,msg_array)
    return matchtuple[0],bids,matchtuple[3]
@partial(jax.jit,static_argnums=0)
...
# Array-based version of ask_lim
@partial(jax.jit,static_argnums=0)
def ask_lim_array(cfg:JAXLOB_Configuration,msg_array,askside,bidside,trades):
    """Array-based function for processing a limit order to ask."""
    matchtuple=_match_against_bid_orders_array(cfg,
                                               bidside,
                                               msg_array[2],  # quantity
                                               msg_array[3],  # price
                                               trades,
    ...
    # Update quantity in array
    msg_array = msg_array.at[2].set(matchtuple[1])
    asks=add_order_array(askside,msg_array)
    return asks,matchtuple[0],matchtuple[3]
...
def match_top_order_if_pricematch(cfg:JAXLOB_Configuration,side,msg,askside,bidside,trades):
    if side==0:
        ...
        match_tuple=(idx, askside, msg['quantity'], msg['price'],
            trades, msg['order_id'], msg['time'], msg['time_ns'],msg["traderid"],msg['side'])
        (top_order_idx,orderside,
        qtm,price,trade,_,_,_,_,_)=_match_ask_order(cfg,match_tuple)
    if side==1:
...
# Array-based version of cond_type_side - eliminates dictionary creation
@partial(jax.jit,static_argnums=(0,))
def cond_type_side_array(config : JAXLOB_Configuration,book_state, it_data):
    """Optimized array-based branching function which calls the relevant function based on
    the side and type fields of the incoming message. Uses direct array access
    instead of creating dictionaries, eliminating Host-to-Device transfers.
        ...
        Parameters:
                book_state (Tuple): State of the orderbook
                    ...
                    bidside (Array): All bid orders in the book
                    trades (Array): All trades which have occured
                it_data (Tuple): (key, data) where data is Array containing message content
    ...
    # Direct array access instead of dictionary creation
    s = data[1]  # side
    t = data[0]  # type
    ...
    if config.simulator_mode == cst.SimulatorMode.GENERAL_EXCHANGE.value:
        ...
        ask, bid, trade = jax.lax.switch(index,
                                        (partial(ask_lim_array,config), partial(bid_lim_array,config),
                                        partial(ask_cancel_array,config,key), partial(bid_cancel_array,config,key),doNothing_array),
                                        data,  # Pass array directly
                                        askside,
                                        bidside,
                                        trades)
    elif config.simulator_mode == cst.SimulatorMode.LOBSTER_INTERPRETER.value:
        # Note: removed print statement to maintain JIT compilation
        index= ((((s == -1) & (t == 1))   ) * 0 
                + (((s ==  1) & (t == 1)) ) * 1
                + (((s == -1) & (t == 2)) | ((s == -1) & (t == 3))) * 2 
                + (((s ==  1) & (t == 2)) | ((s ==  1) & (t == 3))) * 3
        ...
        ask, bid, trade = jax.lax.switch(index,
                                (partial(ask_lim_array,config), partial(bid_lim_array,config),
                                partial(ask_cancel_array,config,key), partial(bid_cancel_array,config,key),
                                partial(match_top_order_if_pricematch_array,config)),
                                data,  # Pass array directly
                                askside,
                                bidside,
                                trades)
...
# Array-based scan function that eliminates dictionary creation
def scan_through_entire_array_optimized(cfg:JAXLOB_Configuration,
                                        key:chex.PRNGKey,
                                        msg_array: chex.Array,
                                        book_state: tuple):
    """Optimized wrapper function around lax.scan which returns only the new 
    state of the orderbook after processing ALL messages using array-based
    functions to eliminate Host-to-Device transfers.
    No intermediate order book states are saved. 
        Parameters:
                msg_array (Array): All messages to process
                book_state (Tuple): State of the orderbook
        ...
        Returns:
                book_state (Tuple): Same as parameter after processing
                    the messages in msg_array.
    """
    keys=jax.random.split(key,msg_array.shape[0] if msg_array.ndim > 0 else 1)  # Handle shape properly
    func=partial(cond_type_side_array,cfg)  # Use optimized array version
    book_state,_=jax.lax.scan(func,book_state,(keys,msg_array))
    return book_state
