...
class ExecutedPairsRecorder():
    ...
    def update(self, pairs):  # to be used in step
        ...
        try:
            if 'market' in [list(pair.keys())[0] for pair in pairs]:
                self.market_pairs[self.index] = np.array(self.market_pairs[self.index]).T
            else:
                pass
            if 'agent' in [list(pair.keys())[0] for pair in pairs]:
                self.agent_pairs[self.index] = np.array(self.agent_pairs[self.index]).T
            else:
                pass
        except:
            raise NotImplementedError
