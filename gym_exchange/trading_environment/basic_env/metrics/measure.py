...
def distance(x,y):
    sign = (x == y)
    result = 2* sign[::2].sum() + 1* sign[::-1].sum() # price, quantity
    return result
...
class OrderbookDistance:
    ...
    def get_distance(self, Self):
        ...
        generated_orderbook = np.array([[generated_orderbook_ask[2*i],generated_orderbook_ask[2*i+1],generated_orderbook_bid[2*i],generated_orderbook_bid[2*i+1]] for i in range(Config.price_level)])
        generated_orderbook = generated_orderbook.flatten().reshape(-1,2)

        result = distance(generated_orderbook, data_orderbook)
        x,y = generated_orderbook, data_orderbook
        result = 0
