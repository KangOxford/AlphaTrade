...
class AttentionBlock(nn.Module):
    ...
    def __call__(self, src, mask):
        # TODO: mask sign is opposite to pytorch
...
class TransformerCritic(nn.Module):
    ...
    def __call__(self, inputs, mask):
        ...
        # mask: batch_size * nhead * seq_len * seq_len
...
class TransformerPolicy(PPOPolicy):
    ...
    # def forward(self, obs: np.ndarray, mask: np.ndarray, deterministic: bool = False) -> np.ndarray:
    #     return self._predict(obs, mask, deterministic=deterministic)

    # def _predict(self, observation: np.ndarray, mask: np.ndarray, deterministic: bool = False) -> np.ndarray:  # type: ignore[override]
    #     # TODO: add mask to select action
    #     if deterministic:
    ...
    def predict_all(self, observation: np.ndarray, mask: np.ndarray, key: jax.Array) -> np.ndarray:
    ...
    def _predict_all(actor_state, vf_state, observation, mask, key):
        ...
        # TODO: add mask to value computation
